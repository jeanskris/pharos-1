<?php
    ...
    */
    ...
    function get_trade_data_by_code($stockcode)
    {
        ...
        {
            $url_yahoo="http://table.finance.yahoo.com/table.csv?s=".$stockcode.".ss";
        }
        else
        {
            $url_yahoo="http://table.finance.yahoo.com/table.csv?s=".$stockcode.".sz";
        }
        //初始化curl
        $ch=curl_init();
        ...
        for ($time=0; $time <10 ; $time++) { 
            ...
            if ( !($yahoo_all_string===false) ) {
                ...
                {
                    if ($i >= count($close_array)-2) {
                        $change_array[$i]=0;
                    }
                    else
                    {
                        $change_array[$i]=((float)$close_array[$i]-(float)$close_array[$i+1])/(float)$close_array[$i+1]*100;
                        $change_array[$i]=round($change_array[$i],2);
                    }
                }
                break;
            }
            //如果十次都没获取成功，给出错误提示
            if ($time==9) {
                echo "cUrl Error in $stockcode yahoo page: ".curl_error($ch)."\n";
                curl_close($ch);
            }
        }
        $result_array=array("tradedate"=>$trade_date_array,"open"=>$open_array,"close"=>$close_array,
            "high"=>$high_array,"low"=>$low_array ,"adjclose"=>$adjclose_array,"change"=>$change_array);
        return $result_array;
    }
    ...
    function get_inf_by_code($stockcode)
    {
            ...
            if(!($string===false)){
                ...
                foreach ($all_inf[0] as $key => $value) {
                    if ( ! preg_match($mode_every_inf, $value, $every_inf )) {
                        $every_inf[0]="";
                        echo "Not Match: get_inf_by_code() match every inf from all inf\n";
                    }
     ...
     //   curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)"); 
        ...
        for ($time=0; $time <10 ; $time++) { 
            ...
            //如果十次都没获取成功，给出错误提示
            if ($time==9) {
                echo "cUrl Error in $stockcode holder page: ".curl_error($ch)."\n";
                curl_close($ch);
            }
        }
        $result_array=array(
            "intro"=>$enInf,"inf"=>$inf_array,
            "date"=>$holder_date,"holder"=>$holder_total,"price"=>$holder_price,
            "fhoder"=>$fhoder_array,"tholder"=>$tholder_array);  
        return $result_array;
    }

        // print_r(get_inf_by_code("600183"));
?>