    ...
    function get_trade_data_by_code($stockcode)
    {
        ...
        for ($time=1; $time <= 30 ; $time++) {
            ...
            if ( !($yahoo_all_string===false) ) {
                ...
                //计算涨跌幅
                for ($index=0; $index <count($close_array) ; $index++) { 
                    if($index==(count($close_array)-1))
                    {
                        $change=100*( (float)$close_array[$index] - (float)$open_array[$index] ) / (float)$open_array[$index];
                    }
                    else
                    {
                        $change=100*( (float)$close_array[$index] - (float)$close_array[$index+1] )/(float)$close_array[$index+1];
                    }
                    $change_array[$index]=round($change,2);
                }
                curl_close($ch);
                break;
            }
